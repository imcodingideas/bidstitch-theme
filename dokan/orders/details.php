...
$statuses = array_diff_key(wc_get_order_statuses(), array_flip(['wc-failed', 'wc-partial-payment']));
        ...
        <div class="dokan-clearfix">
            <div class="" style="width:100%">
                <div class="dokan-panel dokan-panel-default">
                    ...
                    <div class="dokan-panel-body general-details space-y-2">
                        <ul class="list-unstyled order-status space-y-2">
                            <li class="flex items-center space-x-2">
                                <span><?php esc_html_e( 'Order Status:', 'dokan-lite' ); ?></span>
                                <label class="dokan-label dokan-label-<?php echo esc_attr( dokan_get_order_status_class( dokan_get_prop( $order, 'status' ) ) ); ?> text-base font-normal"><?php echo esc_html( dokan_get_order_status_translated( dokan_get_prop( $order, 'status' ) ) ); ?></label>
                                ...
                                <?php if ( current_user_can( 'dokan_manage_order' ) && dokan_get_option( 'order_status_change', 'dokan_selling', 'on' ) == 'on' && $order->get_status() !== 'cancelled' && $order->get_status() !== 'refunded' ) {?>
                                    <a href="#" class="dokan-edit-status bg-black text-white px-2 py-1"><?php esc_html_e( 'Edit', 'dokan-lite' ); ?></a>
                                <?php } ?>
                            </li>
                            <?php if ( current_user_can( 'dokan_manage_order' ) && dokan_get_option( 'order_status_change', 'dokan_selling', 'on' ) == 'on' && $order->get_status() !== 'cancelled' && $order->get_status() !== 'refunded' ): ?>
                                <li class="dokan-hide">
                                    <form class="space-y-2" id="dokan-order-status-form" action="" method="post">
                                        ...
                                        <div class="flex space-x-2">
                                            <input type="submit" class="flex-1 block bg-black text-white px-2 py-1 text-sm text-center cursor-pointer" name="dokan_change_status" value="<?php esc_attr_e( 'Update', 'dokan-lite' ); ?>">
                                            <a href="#" class="dokan-cancel-status flex-1 block bg-gray-300 text-black px-2 py-1 text-sm text-center cursor-pointer"><?php esc_html_e( 'Cancel', 'dokan-lite' ) ?></a>
                                        </div>
                                    </form>
                                </li>
                            <?php endif ?>
                        ...
                        if ( get_option( 'woocommerce_enable_order_comments' ) != 'no' ) {
                            ...
                            if ( !empty( $customer_note ) ) {
                                ?>
                                <div class="p-2 alert alert-success customer-note">
            ...
            <div class="" style="width:100%">
                <div class="dokan-panel dokan-panel-default">
                    ...
                    <div class="dokan-panel-body" id="dokan-order-notes">
                        ...
                        echo '<ul class="order_notes list-unstyled space-y-3">';
                        ...
                        ?>
                        <div class="add_note mt-4">
                            <?php if ( current_user_can( 'dokan_manage_order_note' ) ): ?>
                                <h4><?php esc_html_e( 'Add note', 'dokan-lite' ); ?></h4>
                                <form class="dokan-form-inline" id="add-order-note" role="form" method="post">
                                    <p>
                                        <textarea type="text" id="add-note-content" name="note" class="form-control w-full" cols="19" rows="3"></textarea>
                                    </p>
                                    <div class="clearfix flex flex-col sm:flex-row space-x-0 sm:space-x-2 space-y-2 sm:space-y-0">
                                        <div class="order_note_type flex-grow">
                                            <select name="note_type" id="order_note_type" class="w-full">
                                        ...
                                        <input type="submit" name="add_order_note" class="add_note bg-black text-white px-4 py-2 cursor-pointer" value="<?php esc_attr_e( 'Add Note', 'dokan-lite' ); ?>">
                                    </div>
                                </form>
                            <?php endif; ?>
                            ...
                            <div class="clearfix dokan-form-group" style="margin-top: 10px;">
                                ...
                                <input type="button" id="dokan-add-tracking-number" name="add_tracking_number" class="dokan-btn dokan-btn-success w-full text-base" value="<?php esc_attr_e( 'Tracking Number', 'dokan-lite' ); ?>">
                                ...
                                <form id="add-shipping-tracking-form" method="post" class="dokan-hide" style="margin-top: 10px;">
                                    ...
                                    <div class="dokan-form-group flex flex-col sm:flex-row space-x-0 sm:space-x-2 space-y-2 sm:space-y-0">
                                        <input id="add-tracking-details" type="button" class="bg-black text-white font-normal px-4 py-2 cursor-pointer" value="<?php esc_attr_e('Add Tracking Details', 'dokan-lite' );?>">
                                        <button type="button" class="flex-grow bg-gray-300 text-black font-normal px-4 py-2 cursor-pointer" id="dokan-cancel-tracking-note"><?php esc_html_e( 'Close', 'dokan-lite' );?></button>
                                    </div>
                                </form>
                            </div>
                        </div> <!-- .add_note -->
