<div class="clear grid space-y-4 bg-gradient-to-r from-white via-gray-100 to-white py-8 my-8 border-t border-b md:py-12 md:my-12">
  <div class="grid items-center justify-between w-full space-y-3 md:flex md:space-y-0">
    <div class="flex items-start space-x-3 md:space-x-6 md:align-center">
      <a href="{{ $store_url }}" class="block bg-gray-100">
        <img src="{{ esc_url($store_user->get_avatar()) }}" alt="{{ esc_attr($store_user->get_shop_name()) }}" size="64" class="shadow-sm rounded-sm h-28 w-28">
      </a>
    ...
    <div class="flex items-center space-x-3 md:space-x-6">
      ...
      @else
        <a href="{{ home_url('/log-in') }}" class="btn btn--white px-4 py-2 uppercase md:px-8">{{ _e('Follow', 'sage') }}</a>
      @endif
      <a href="{{ $store_url }}" class="btn btn--white px-4 py-2 uppercase md:px-8">{{ _e('View more', 'sage') }}</a>
    </div>
  </div>
</div>