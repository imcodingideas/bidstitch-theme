  ...
  <tr class="transition-colors bg-white hover:bg-gray-50 grid gap-y-4 p-4 sm:table-row sm:p-0">
    ...
    <td
      ...
      @if ($can_export_to_shopify)
        ...
        <div x-data="shopifyExportData()" x-init="init({{ $product->id }}, {{ $exported_to_shopify }})" class="col-span-2 sm:mt-2 sm:ml-2 text-center sm:text-left">
          <a x-on:click.prevent="exportProduct" class="text-base sm:text-sm hover:underline" :class="(exported || exporting) ? 'text-gray-400' : 'text-gray-700'" href="#">
