    ...
    <td
      ...
      @if (current_user_can($is_auction ? 'dokan_delete_auction_product' : 'dokan_delete_product'))
        <a onClick="return confirm('{{ _e('Are you sure you want to delete?', 'sage') }}');"
          class="btn btn--white btn--sm bg-red-600 hover:bg-red-600 text-white justify-center" href="{!! $delete_url !!}">{{ _e('Delete', 'sage') }}</a>
      @endif
      @if (current_user_can($is_auction ? 'dokan_delete_auction_product' : 'dokan_delete_product'))
      ...
      @if ($can_export_to_shopify)
        <div x-data="shopifyExportData()" x-init="init({{ $product->id }}, {{ $exported_to_shopify }})" class="sm:mt-2 sm:ml-2 col-span-2">
          <a x-on:click.prevent="exportProduct" class="btn btn--white btn--sm justify-center w-full sm:w-auto text-xs" :class="(exported || exporting) ? 'bg-gray-200 text-gray-400' : ''" href="#">
            <div>
              <svg class="inline-block w-5 h-5 fill-current"><use href="#icon-shopify"></use></svg> <span x-text="exported ? 'Exported' : 'Export'"></span> to shopify
            </div>
          </a>
        </div>
      @endif
