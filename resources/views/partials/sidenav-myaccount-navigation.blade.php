@if ($navigation)
  <li class="border-b">
    <button
      class="w-full px-6 py-3 flex items-center text-left justify-between focus:outline-none sidenav__dropdown__toggle">
    ...
    <ul class="sidenav__dropdown__menu hidden bg-gray-50">
      @foreach ($navigation as $item)
        <li class="border-t">
          <a class="flex px-6 py-3 text-sm font-bold {{ $item->classes ?? '' }} {{ $item->active ? 'active' : '' }}"
            href="{{ $item->url }}" target="{{ $item->target }}">{{ $item->label }}</a>
        </li>
        @if ($item->children)
          @foreach ($item->children as $child)
            <li>
              <a class="flex px-6 py-3 text-sm {{ $child->classes ?? '' }} {{ $child->active ? 'active' : '' }}"
